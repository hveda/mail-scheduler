...
def test_create_app_testing_config():
    ...
    assert app.static_folder.endswith('static')  # Full path will vary by environment
...
def test_create_app_registers_components(mock_commands, mock_blueprints, mock_extensions):
...
def test_register_blueprints_registers_api():
    ...
    mock_app.register_blueprint.assert_any_call(mock_event_blueprint, url_prefix='/items')
...
def test_app_configuration_environment_vars():
    ...
    # Create app with mocked environment variables
    with patch('os.environ.get') as mock_env_get:
        # Configure the mock to return our test values when requested
        ...
        test_config = type('TestEnvConfig', (config.Config,), {})
