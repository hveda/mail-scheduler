...
def mock_recipient(monkeypatch):
    ...
    class MockRecipient:
        def __init__(self, email, event_id, name=None, is_sent=False, sent_at=None):
            self.email = email
            self.email_address = email  # For backward compatibility with tests expecting email_address
...
def test_add_recipients(mock_db_session, mock_recipient):
    ...
    for i, call_args in enumerate(mock_db_session.add.call_args_list):
        ...
        assert recipient.email_address in ["test1@example.com", "test2@example.com", "test3@example.com"]
...
def test_schedule_mail(monkeypatch):
    ...
    mock_scheduler.enqueue_at.assert_called_once_with(timestamp, send_mail, event_id, recipients)
...
def test_send_mail_html_content(mock_event_query, mock_db_session, mock_mail_connection, monkeypatch):
    ...
    mock_msg.add_recipient.assert_has_calls([call("test@example.com"), call("another@example.com")])
...
def test_add_event(mock_db_session, monkeypatch):
    ...
    mock_schedule_mail.assert_called_once_with(mock_event.id, ["test@example.com"], test_datetime)
