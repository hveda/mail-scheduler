...
def test_render_template_function(app, client):
    ...
    with app.test_request_context():
        with patch(
            "app.event.views.render_template", side_effect=render_template
        ) as mock_render:
...
def test_index_view(app, client):
    ...
    with app.test_request_context():
        ...
        # 302 if redirect due to login required
    ...
    with app.test_request_context():
        ...
        with patch("flask_login.utils._get_user") as mock_get_user:
            ...
            response = client.post(
                url_for("items.add_event"), data=data, follow_redirects=True
            )
            # Should be successful now that we're authenticated
...
def test_event_detail_view(app, client, db):
    ...
    with app.test_request_context():
        ...
        # Either success or redirect due to login required
