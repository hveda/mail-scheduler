    ...
    def post(self):
        ...
        if form.validate_on_submit():
            try:
                ...
                message = Markup("<strong>Well done!</strong> Email scheduled successfully!")
        ...
        if form.validate_on_submit():
            try:
                ...
                if not event.is_done:
                    ...
                    message = Markup("Cannot edit an email that has already been sent.")
...
class EventDeleteView(MethodView):
    ...
    def get(self, event_id):
        ...
        event = EventService.get_event_by_id(event_id)
        if event is None:
            message = Markup("<strong>Error!</strong> Scheduled email does not exist.")
            flash(message, 'danger')
            return redirect(url_for('items.all_events'))
    ...
    def post(self, event_id):
        ...
        try:
            ...
            message = Markup(f"<strong>Done.</strong> Scheduled email has been deleted.")
            flash(message, 'success')
        except Exception as e:
            db.session.rollback()
            message = Markup(f"<strong>Error!</strong> Could not delete email: {str(e)}")
            flash(message, 'danger')
...
blueprint.add_url_rule('/add_event', view_func=event_add_view, methods=['GET', 'POST'])
blueprint.add_url_rule('/edit_event/<int:event_id>', view_func=event_edit_view, methods=['GET', 'POST'])
blueprint.add_url_rule('/delete_event/<int:event_id>', view_func=event_delete_view, methods=['GET', 'POST'])
